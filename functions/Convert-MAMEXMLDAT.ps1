function Convert-MAMEXMLDAT {
    ...
    process {
        ...
        $Result = Select-Xml -Xml ${XML} -XPath ${XMLPath} | Select-Object -ExpandProperty Node | Where-Object { $_.category -eq "Games" } | Where-Object { $_.Name -like "*(USA)*" } | Select-Object Name, @{name="Description"; expression={ $_.Description.Replace(" (USA)","") }} | Sort-Object -Property Name
        Return $Result
    }
